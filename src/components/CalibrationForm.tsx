...
export default function CalibrationForm({ initialData, mode, id }: Props) {
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} onKeyDown={onFormKeyDown} className="space-y-6">
      ...
      {data.approvalStatus === 'rejected' && (initialData as any)?.rejectionComment && (
        <div className="card border border-red-300 bg-red-50 text-red-900">
          <div className="flex items-start gap-2">
            ...
            <div>
              <p className="font-semibold">รายการนี้ไม่ได้รับการอนุมัติ</p>
              <p className="text-sm mt-1">เหตุผล: {(initialData as any).rejectionComment}</p>
              {(initialData as any).rejectedByName && (
                <p className="text-xs text-red-700 mt-1">โดย: {(initialData as any).rejectedByName}</p>
              )}
              <p className="text-sm mt-2">กรุณาแก้ไขข้อมูลและส่งอนุมัติใหม่</p>
            </div>
          </div>
        </div>
      )}
